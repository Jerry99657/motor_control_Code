    ...
    else
    {
      ...
      for (index = 0; index < (sizeof(OV5640_Common) / 4U) ; index++)
      {
        if (ret != OV5640_ERROR)
        {
          ...
          else if ((OV5640_Common[index][0] == OV5640_SYSTEM_CTROL0) &&
                   (tmp == 0x82U))
          {
            /* The sensor needs time after its software reset before SCCB writes. */
            (void)OV5640_Delay(pObj, 5U);
          }
        }
      }
...
static int32_t OV5640_ModifyRegWrap(void *handle, uint16_t Reg, uint16_t Mask, uint8_t *pData, uint16_t Length)
{
  ...
  if ((pData == NULL) || (Length != 1U) ||
      (pObj->IO.ReadReg(pObj->IO.Address, Reg, &TempData1, Length) != OV5640_OK))
  {
    return OV5640_ERROR;
  }

  TempData1 = (uint8_t)((TempData1 & (uint8_t)(~Mask)) |
                        ((*pData) & (uint8_t)Mask));
