...
`HOLD RESET` 是独立的软复位按钮。持续按住 1 秒，进度填满并显示 `RESET SENT` 后，当前 ROM 从复位向量重新启动，通常会回到游戏标题画面。短按不会触发；该操作不退出模拟器，也不写入或删除 `.sav`。
...
```text
手机 -> ESP32: 52 sequence
ESP32 -> STM32: 77 68 07 0E 03 sequence 0A
```
...
6. 短按 `HOLD RESET` 应无动作；长按 1 秒应返回当前游戏的标题画面，但不能退出到 SD 页面。
7. 验证方向键+A/B 多点触控。
8. 同时使用手机和实体按键，确认可以交叉组合。
9. 按住手机方向键后关闭网页或断开 Wi-Fi，确认约 350 ms 后方向自动释放，实体按键仍可操作。
10. 按 KEY3 返回原 SD 目录，并确认 `.sav` 行为保持不变。
